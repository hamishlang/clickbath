...
function synthSetup(theButton, panName, targetTwo, number) {
  ...
  theButton.on('click', e => {
    ...
    $(targetTwo).css('background', 'radial-gradient(at ' + gradientPercX + '% ' + gradientPercY + '%, rgba(255, 255, 255, 0.5)0%, rgba(101,141,210,0.0) 41%)'); // $(theButton).css(
...
popupMaker(".area2", ".areaShow2"); // $('.area').mousedown(function (e) {
//   switch (e.which) {
//     case 1:
...
function knobFunction(target, parent, toneName) {
  ...
  document.getElementById(target).addEventListener("input", event => {
    ...
    let volumeKnobEquation = event.target.value * 0.5 - 50; // console.log(knobAdjust);

    $(parent).css('background-color', 'rgba(100,' + knobAdjust + ', 200, 0.3)');
    toneName.volume.value = volumeKnobEquation; // console.log(event.target.value)

    if (event.target.value > 0) {
      toneName.mute = false; // toneName.autostart = true;
...
cursorForBoxes(area2, cursor2); // $('.options').toggle(function () {
